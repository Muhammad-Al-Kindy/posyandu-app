...
<div class="container fs-normal">
  ...
  <div class="row">
    ...
    <div class="col-md-6 d-flex justify-content-end">
      <a href="/immunization/{{ $baby->id }}/show" class="btn btn-primary mx-2 py-2 shadow-sm fs-normal align-self-center px-3 mt-n3">
            ...
            <div class="col-md-4">
              ...
              <select name="id_vaccine" id="id_vaccine" class="form-control-select fs-normal form-spacer-10x8 @error('id_vaccine') is-invalid @enderror">
                @foreach ($vaccines as $item)
                  <option value="{{ $item->id }}" {{ (old('id_vaccine') == $item->id) ? 'selected' : '' }}>{{ $item->name }}</option>
                @endforeach
              </select>
